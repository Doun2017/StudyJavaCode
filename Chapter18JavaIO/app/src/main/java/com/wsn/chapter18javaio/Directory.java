/*: net/mindview/util/Directory.java Produce a sequence of File objects that match a regular expression in either a local directory, or by walking a directory tree.*/
...
public final class Directory {
    ...
    static TreeInfo recurseDirs(File startDir, String regex) {
        ...
        for (File item : startDir.listFiles())
            ...
            } else /* Regular file*/ if (item.getName().matches(regex)) result.files.add(item);
        return result;
...
//        else for (String arg : args) System.out.println(walk(arg));
        ...
        for (File file:treeInfo.files){
            allSizeSum += file.length();
            System.out.println("+ "+file.getName()+"  =  "+allSizeSum);
        }
